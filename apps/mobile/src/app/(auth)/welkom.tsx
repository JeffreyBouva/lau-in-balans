  ...
  return (
    <View style={[s.root, { paddingTop: insets.top + 60, paddingBottom: insets.bottom + 30 }]}>
      <View style={s.boven}>
        ...
        <Text style={text.onboardingHero}>Rust in je eetritme.</Text>
        ...
        <View style={s.punten}>
          {PUNTEN.map((p) => (
            <View key={p.tekst} style={s.punt}>
              {/* Puur decoratief vinkje: screenreaders slaan 'm over. */}
              <Text style={s.puntIcoon} accessible={false}>{p.icoon}</Text>
      ...
      <View style={s.acties}>
        ...
        <Pressable onPress={() => router.push('/(auth)/login')} style={s.loginLink} accessibilityRole="button">
